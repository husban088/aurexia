...
function OrderSuccessUI({
  ...
  useEffect(() => {
    ...
    const COLORS = [
      "#daa520", "#f5c842", "#fff8e1",
      "#b8860b", "#ffe082", "#c8a415", "#ffd54f",
    ];
    ...
    const startDelay = setTimeout(() => { frame = requestAnimationFrame(draw); }, 350);
    return () => { clearTimeout(startDelay); cancelAnimationFrame(frame); };
  }, []);

  // ✅ Notification status display
  const renderNotifStatus = (sent: boolean | null, type: "email" | "whatsapp") => {
    if (sent === null) {
      return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", color: "#888", fontSize: "13px" }}>
          <span style={{ width: "12px", height: "12px", border: "2px solid #daa520", borderTopColor: "transparent", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} />
          Sending...
        </span>
      );
    }
    if (sent === true) {
      return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", color: "#22c55e", fontSize: "13px", fontWeight: 600 }}>
          ✅ Sent Successfully
        </span>
      );
    }
    return (
      <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", color: "#f59e0b", fontSize: "13px" }}>
        ⚠️ Not delivered
      </span>
    );
  };
  ...
  return (
    <div className={`os-root ${visible ? "os-root--visible" : ""}`}>
      ...
      <canvas ref={confettiRef} className="os-confetti-canvas" aria-hidden="true" />
      ...
      <div className="os-wrap">
        ...
        <div className="os-left">
          ...
          <p className="os-sub">
            Thank you <strong>{firstName}</strong>! Your order has been placed successfully.
          </p>

          {/* Notification Status */}
          <div style={{ background: "#f9f9f9", border: "1px solid #eee", borderRadius: "12px", padding: "16px 20px", marginBottom: "24px" }}>
            <p style={{ margin: "0 0 12px", fontWeight: 600, fontSize: "14px", color: "#333" }}>
              📬 Notifications Sent To:
            </p>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              <span style={{ fontSize: "13px", color: "#555" }}>📧 Email: <strong>{email}</strong></span>
              {renderNotifStatus(notifStatus.email, "email")}
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: "13px", color: "#555" }}>💬 WhatsApp: <strong>{fullPhone}</strong></span>
          ...
          <div className="os-details-card">
            ...
            <div className="os-detail-item">
              ...
              <span className="os-detail-val" style={{ fontFamily: "monospace", color: "#daa520" }}>{orderNumber}</span>
            </div>
            <div className="os-detail-item">
                ...
                {paymentMethod === "card" ? (
                  <>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="18" height="18">
                      <rect x="2" y="4" width="20" height="16" rx="2" /><path d="M2 8h20M7 16h2M13 16h4" />
                    </svg>
                  ...
                  <>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="18" height="18">
                      <path d="M7 8h10M7 12h6M7 16h4" /><rect x="3" y="4" width="18" height="16" rx="2" />
                    </svg>
                    PayPal
                  </>
                )}
          ...
          <div className="os-timeline">
            <div className="os-timeline-item os-timeline-item--done">
              ...
              <div className="os-tl-text"><strong>Order Placed</strong><span>Confirmed &amp; payment received</span></div>
            </div>
            <div className="os-timeline-item">
              <div className="os-tl-dot os-tl-dot--pulse" />
              <div className="os-tl-text"><strong>Processing</strong><span>Your items are being prepared</span></div>
            </div>
            <div className="os-timeline-item os-timeline-item--muted">
              <div className="os-tl-dot os-tl-dot--empty" />
              <div className="os-tl-text"><strong>Shipped</strong><span>You'll receive tracking info via WhatsApp</span></div>
            </div>
            <div className="os-timeline-item os-timeline-item--muted">
              <div className="os-tl-dot os-tl-dot--empty" />
              <div className="os-tl-text"><strong>Delivered</strong><span>Enjoy your luxury purchase</span></div>
            </div>
          </div>
          ...
          <div className="os-summary-card">
            <div className="os-summary-header">
              <p className="os-summary-title">
                <span className="os-ey-line" />Order Summary<span className="os-ey-line" />
              </p>
            ...
            <ul className="os-items-list">
              {items.map((item) => {
                const product = item.product ?? { name: item.variant_name || "Product", images: item.variant_image ? [item.variant_image] : [], price: item.variant_price ?? 0 };
                ...
                const displayName = item.variant_name && item.variant_name !== "Standard"
                ...
                return (
                  <li key={item.id} className="os-item">
                    <div className="os-item-img">
                      {displayImage ? (
                        <img src={displayImage} alt={productName} style={{ objectFit: "cover", width: "100%", height: "100%", borderRadius: "8px" }} />
                      ) : (
                    ...
                    <div className="os-item-info">
                      ...
                      <p className="os-item-qty">
                        ...
                        {ppu > 1 && (
                          <span style={{ display: "block", fontSize: "0.7rem", opacity: 0.65, marginTop: "2px" }}>
            ...
            <div className="os-perks">
              <div className="os-perk"><span className="os-perk-icon">🔒</span><span>Secure Checkout</span></div>
              <div className="os-perk"><span className="os-perk-icon">🚚</span><span>Free Shipping</span></div>
              <div className="os-perk"><span className="os-perk-icon">↩</span><span>30-Day Returns</span></div>
              <div className="os-perk"><span className="os-perk-icon">✦</span><span>Luxury Packaging</span></div>
            </div>
...
export default function OrderSuccessPage() {
  ...
  const [notifStatus] = useState<{ email: boolean | null; whatsapp: boolean | null }>(
  ...
  useEffect(() => {
    ...
    if (!raw) {
      // ✅ Koi valid order data nahi — home pe redirect karo
      router.replace("/");
      return;
    }
  ...
  if (!ready || !orderData) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#faf8f3" }}>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
        <div style={{ width: 44, height: 44, border: "3px solid #f3f3f3", borderTop: "3px solid #daa520", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
      </div>
    );
  }

  // ✅ Sab kuch ready — OrderSuccess render karo
  return (
    <OrderSuccessUI
