...
// ✅ Shipped → Courier info Supabase mein save + WhatsApp+Email mein full tracking details
// ✅ Delivered / Cancelled → WhatsApp + Email notification
// ✅ Self-contained — koi bhi import fail nahi hoga
...
async function sendWA(to: string, message: string): Promise<boolean> {
  ...
  if (!apiKey) { console.warn("⚠️ WASENDER_API_KEY missing"); return false; }
  ...
  try {
    ...
    if (res.ok && r.success) { console.log(`✅ WA sent → ${toF}`); return true; }
    console.error("❌ WA error:", JSON.stringify(r));
    return false;
  } catch (e: any) { console.error("❌ WA fetch error:", e.message); return false; }
}
...
async function sendEmail(to: string, subject: string, html: string, text: string): Promise<boolean> {
  try {
    ...
    await t.sendMail({ from: `"Tech4U Orders" <${from}>`, to, replyTo: from, subject, html, text });
...
⏱ *Estimated Delivery:* ${shipping?.estimatedDays || "2–5 business days"}${
      shipping?.riderNumber
        ? `\n🏍️ *Rider/Driver Number:* ${shipping.riderNumber}`
        : ""
    }
...
function buildEmailHtml(
  ...
  const cfg = {
    shipped: {
      emoji: "🚚", label: "Shipped", color: "#1565c0", bg: "#e3f2fd",
      ...
      ${shipping?.riderNumber ? `<strong>🏍️ Rider/Driver:</strong> ${shipping.riderNumber}<br>` : ""}
      <br>Thank you for shopping with Tech4U! 😊`,
    },
    delivered: {
      emoji: "✅", label: "Delivered", color: "#1b5e20", bg: "#e8f5e9",
    ...
    cancelled: {
      emoji: "❌", label: "Cancelled", color: "#b71c1c", bg: "#ffebee",
        ...
        <tr>
          <td style="padding:20px 32px;">
            <table width="100%" cellpadding="8" cellspacing="0" style="background:#f9f6f0;border-radius:12px;">
              ...
              ${status === "shipped" && shipping?.riderNumber ? `
              <tr>
                <td style="font-size:13px;color:#666;">Rider/Driver</td>
                <td style="font-size:13px;font-weight:700;color:#1a1a1a;text-align:right;">${shipping.riderNumber}</td>
              </tr>` : ""}
            </table>
          </td>
        </tr>
...
function getEmailSubject(status: OrderStatus, orderNumber: string): string {
  if (status === "shipped") return `🚚 Your Order #${orderNumber} Has Been Shipped — Tech4U`;
  if (status === "delivered") return `✅ Your Order #${orderNumber} Has Been Delivered — Tech4U`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      orderId, status,
      customerEmail, customerPhone, customerName, orderNumber,
      // Shipping fields (only for "shipped")
      courierName, courierCountry, estimatedDays, riderNumber,
    } = body ?? {};
    ...
    const shipping: ShippingInfo = { courierName, courierCountry, estimatedDays, riderNumber };

    console.log("====================================");
    console.log(`🔄 ${typedStatus.toUpperCase()} — #${displayOrderNum}`);
    console.log(`👤 ${displayName} | 📧 ${customerEmail} | 📱 ${customerPhone}`);
    if (typedStatus === "shipped") {
      console.log(`🚚 Courier: ${courierName} (${courierCountry}) | ETA: ${estimatedDays} | Rider: ${riderNumber}`);
    }
    console.log("====================================");
    ...
    if (typedStatus === "shipped") {
      ...
      if (riderNumber) updatePayload.rider_number = riderNumber;
    ...
    if (dbError) {
      // Agar courier columns exist nahi karte toh sirf status update karo
      if (dbError.message.includes("column") || dbError.code === "42703") {
        console.warn("⚠️ Courier columns not in DB yet — saving only status");
    ...
    // ── Step 2: WhatsApp ──
    if (customerPhone) {
      const msg = buildWAMessage(typedStatus, displayName, displayOrderNum, shipping);
    ...
    if (customerEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail)) {
      ...
      const html = buildEmailHtml(typedStatus, displayName, displayOrderNum, shipping);
      const text = buildWAMessage(typedStatus, displayName, displayOrderNum, shipping);
